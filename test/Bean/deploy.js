...
Data.deploy(privateKey,[proxyAddress]).then(function(BeanData,err){
    Data.save()
    console.log(BeanData.address)
    Bean.setAddress(BeanData.address)
    Bean.init([issuer,symbol,0,20000000000,2,100000000,0,"测试分红豆","0xabcdf",Contract.getAddress("TokenManagerData")],privateKey).then(function(receipt,err){
